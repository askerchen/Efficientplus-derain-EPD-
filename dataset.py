...
class RandomCrop(object):
    ...
    def __call__(self, img):
        if len(img.shape) == 3:
            return img[self.h1: self.h2, self.w1: self.w2, :] # RGB图像 HxWx3
        else:
...
class DenoisingDataset(Dataset):
    def __init__(self, opt):                                   		    # root: list ; transform: torch transform
        self.opt = opt
        self.imglist = utils.get_files(opt.baseroot)  # imglist = [[rainy image, gt], [rainy image, gt] ...]
    ...
    def rain_aug(self, img_rainy, img_gt):
        img_rainy = (img_rainy.astype(np.float32)) / 255.0  #  8位深度 2**8 = 256 归一化
        ...
        #rainlayer_rand2ex = self.gXetRandRainLayer2()
        ...
        # fog------------------------------
        (row, col, chs) = img_rainy_ret.shape
        A = 0.8 # 亮度
        beta = random.random() / 32 #雾的浓度 这里还是不固定了也就是0-0.03左右随机
        ...
        for j in range(row):
            for l in range(col):
                ...
                td = math.exp(-beta * d)
                img_rainy_ret[j][l][:] = img_rainy_ret[j][l][:] * td + A * (1 - td)
        # fog-------------------------------
    ...
    def __getitem__(self, index):
        ## read an image
        img_rainy = cv2.imread(self.imglist[index][0])  # HxWxC
        img_gt = cv2.imread(self.imglist[index][1])
        img_rainy = cv2.cvtColor(img_rainy, cv2.COLOR_BGR2RGB)  # 调换通道顺序， 默认 BGR 变为 RGB
        ...
        # random rotate and horizontal flip 随机旋转和水平翻转
        ...
        img_rainy = torch.from_numpy(img_rainy.transpose(2, 0, 1)).contiguous()  # HxWxC --> CxHxW
        img_gt = img_gt / 255.0
...
class DenoisingValDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print("img_rainy : {}".format(img_rainy))
        ...
        img_rainy = img_rainy / 255.0  # HxWxC
        img_rainy = torch.from_numpy(img_rainy.transpose(2, 0, 1)).contiguous()  # CxHxW
        img_gt = img_gt / 255.0
        img_gt = torch.from_numpy(img_gt.transpose(2, 0, 1)).contiguous()  # CxHxW
