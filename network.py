...
class SE_block(nn.Module):
    ...
    def __init__(self, channel):
        ...
        self.se = nn.Sequential(
            ...
            nn.Conv2d(channel, channel//16 if channel >= 64 else channel, kernel_size=1),  # kernel_size = 1 相当于全连接 16是论文中设置的数，目的是减小参数
            nn.ReLU(),
...
class KPN(nn.Module):
    def __init__(self, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
        ...
        self.se1 = SE_block(channel=64)
        self.se2 = SE_block(channel=128)
        self.se3 = SE_block(channel=256)
        self.se4 = SE_block(channel=512)
        self.se_final = SE_block(channel=12)  # cxq
    # 前向传播函数
    def forward(self, data_with_est, data, white_level=1.0):
        ...
        pred1 = self.kernel_pred(data, core, white_level, rate=1)  # pred_image1
        pred2 = self.kernel_pred(data, core, white_level, rate=2)  # pred_image2
        pred3 = self.kernel_pred(data, core, white_level, rate=3)  # pred_image3
        pred4 = self.kernel_pred(data, core, white_level, rate=4)  # pred_image4

        pred_cat = torch.cat([torch.cat([torch.cat([pred1, pred2], dim=1), pred3], dim=1), pred4], dim=1)  # concate -> (N, 3x4 = 12, H, W)
        pred_cat = self.se_final(pred_cat)  # cxq
