...
def create_generator(opt):
    ...
    if opt.load_name == '':
        ...
        if opt.no_gpu:
            pretrained_net = torch.load(opt.load_name, map_location="cpu")  # cxq add map_lacation="cpu"
        else:
...
def save_sample_png(sample_folder, sample_name, img_list, name_list, pixel_max_cnt = 255, height = -1, width = -1):
    ...
    for i in range(len(img_list)):
        ...
        # print("img_copy : {}".format(img_copy))

        if (height != -1) and (width != -1):
            # print("height : {}, width : {}".format(height.item(), width.item()))
            # img_copy = cv2.resize(img_copy, (width.item(), height.item()))
            # img_copy = cv2.resize(img_copy, (481, 321))
            img_copy = cv2.resize(img_copy, (int(width.item()), int(height.item())))  # debug .item() debug int()
