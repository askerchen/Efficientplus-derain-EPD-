...
if __name__ == "__main__":
    ...
    # version, Saving, and loading parameters
    parser.add_argument('--version', type=str, default='./v3sp', help='the version of the trained model')
...
#    os.environ["CUDA_VISIBLE_DEVICES"] = "1"
    ...
    for i, (true_input, true_target, height_origin, width_origin) in enumerate(test_loader):
        ...
        utils.save_sample_png(sample_folder=sample_folder,
                              sample_name='%d' % (i + 1), img_list=img_list, name_list=name_list,
                              pixel_max_cnt=255, height=height_origin, width=width_origin)
        ...
        # print(f"shape : {img_gt_recover.shape}") # HxWxC
        ssim_sum = ssim_sum + structural_similarity(img_gt_recover, img_pred_recover, channel_axis=2, data_range=255)
        eval_cnt = eval_cnt + 1

    end_time = time.time()
    time_ave = (end_time - start_time) / eval_cnt
    psnr_ave = psnr_sum / eval_cnt
    ssim_ave = ssim_sum / eval_cnt


    # psnr_file = "./results/data/psnr.txt"
    # ssim_file = "./results/data/ssim.txt"
    # psnr_content = opt.load_name + ": " + str(psnr_ave) + "\n" + "\n"
    # ssim_content = opt.load_name + ": " + str(ssim_ave) + "\n" + "\n"

    version = opt.version
    save_path = "./test_results/" + version

    if os.path.exists(save_path) == False:
        os.makedirs(save_path)

    psnr_file = save_path + "/psnr.txt"
    ssim_file = save_path + "/ssim.txt"
    time_file = save_path + "/time.txt"

    # psnr_content = opt.load_name + ": " + str(psnr_ave) + "\n"
    # ssim_content = opt.load_name + ": " + str(ssim_ave) + "\n"
    psnr_content = str(psnr_ave) + "\n"
    ssim_content = str(ssim_ave) + "\n"
    time_content = str(time_ave) + "\n"

    # print(psnr_file);

    utils.text_save(content=psnr_content, filename=psnr_file)
    utils.text_save(content=ssim_content, filename=ssim_file)
    utils.text_save(content=time_content, filename=time_file)